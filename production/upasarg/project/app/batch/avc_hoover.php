...
while($page > 0) {
    ...
    if(count($articles)) {
        ...
        for($i=1; $i<=count($articles); $i++) {
            ...
            $titles = $finder->query("//article[position()=$i]/h2/a");
...
//                 continue;
            ...
            $times = $finder->query("//article[position()=$i]/time");
            $new_articles[$url]['datetime_text'] = $times[0]->nodeValue;

            $categories = $finder->query("//article[position()=$i]/div[contains(@class, 'Post-categories')]/a");
            $new_articles[$url]['category'] = $categories[0]->nodeValue;

            $contents = $finder->query("//article[position()=$i]/div[contains(@class, 'ContentArea')]");
...
//                 $ins_query = "INSERT INTO articles (user_id, category, source_site, title, content, url_slug,
//                             orig_url, datetime_text, article_date, date_create) VALUES ";
                $url_slug = trim(preg_replace('/https:\/\/avc\.com\/(\d+)\/(\d+)\//', '', $url), '/');
                $ins_query .= "(
                                {$userid},
